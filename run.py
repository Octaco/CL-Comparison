...
def train(args, model, optimizer, training_set, valid_set):
    ...
    for epoch in tqdm(range(1, args.num_train_epochs + 1), desc="Epochs", dynamic_ncols=True, position=0, leave=True, ascii=True):
        ...
        progress_bar = tqdm(train_dataloader, desc=f"Epoch {epoch} Train", position=1, leave=True, dynamic_ncols=True,ascii=True)
        ...
        all_val_losses = []
        progress_bar = tqdm(validation_dataloader, desc=f"Epoch {epoch} eval ", position=2, leave=True, dynamic_ncols=True, ascii=True)
        for idx, batch in enumerate(progress_bar):
            if batch['code_ids'].size(0) < args.train_batch_size:
                logging.debug("continue")
                continue
...
def evaluation(args, model, valid_set):
    ...
    all_distances = []
    progress_bar = tqdm(eval_dataloader, position=0, leave=True, dynamic_ncols=True)
    for idx, batch in enumerate(progress_bar):
