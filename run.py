...
def train(args, model, optimizer, training_set, valid_set):
    ...
    for epoch in tqdm(range(1, args.num_train_epochs + 1),desc="Epochs", position=0, leave=True):
        ...
        progress_bar = tqdm(train_dataloader, desc=f"Epoch {epoch} Train", position=1, leave=True)
